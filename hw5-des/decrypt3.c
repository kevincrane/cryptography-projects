...
// Decrypts the input using EDE triple DES driven by tablefile in the ECB mode 
...
void decrypt3(char *key, FILE *table, FILE *input) {
	...
	while((size = fread(input_block, 1, 8, input))) {
		...
		decrypt_block(input_block, cipher_block, keys3, table, first);
		
		for(int i=0; i<8; i++) input_block[i] = 0x00;
		encrypt_block(cipher_block, input_block, keys2, table, first);
		
		for(int i=0; i<8; i++) cipher_block[i] = 0x00;
		decrypt_block(input_block, cipher_block, keys1, table, first);
